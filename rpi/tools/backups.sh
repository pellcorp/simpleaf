...
while true; do
    if [ "$1" = "--create" ]; then
        ...
        if [ "$restore" = "latest" ]; then
            if [ -d $HOME/printer_data/config/backups ] && [ $(ls -lt $HOME/printer_data/config/backups/*.tar.gz 2> /dev/null | wc -l) -gt 0 ]; then
                restore=$(ls -lt $HOME/printer_data/config/backups/*.tar.gz 2> /dev/null | head -1 | awk '{print $9}' | awk -F '/' '{print $7}')
            else
        ...
        if [ ! -f $HOME/printer_data/config/backups/$restore ]; then
            echo "ERROR: Backup $HOME/printer_data/config/backups/$restore not found!"
            exit 1
        fi
...
if [ "$mode" = "create" ]; then
    ...
    tar -zcf $HOME/printer_data/config/backups/backup-${TIMESTAMP}.tar.gz $CFG_ARG $CONF_ARG $PELLCORP_BACKUPS $PELLCORP_OVERRIDES $PELLCORP_DONE
    sync
    ...
    if [ -d $HOME/printer_data/config/backups ] && [ $(ls -lt $HOME/printer_data/config/backups/*.tar.gz 2> /dev/null | wc -l) -gt 0 ]; then
        latest=$(ls -lt $HOME/printer_data/config/backups/*.tar.gz 2> /dev/null | head -1 | awk '{print $9}' | awk -F '/' '{print $7}')
        if [ -n "$latest" ]; then
            echo "$latest"
            exit 0
        else
    ...
    if [ -d $HOME/printer_data/config/backups ] && [ $(ls -lt $HOME/printer_data/config/backups/*.tar.gz 2> /dev/null | wc -l) -gt 0 ]; then
        ls -lt $HOME/printer_data/config/backups/*.tar.gz 2> /dev/null | awk '{print $9}' | awk -F '/' '{print $7}'
        exit 0
    else
        echo "ERROR: No backups found"
        exit 1
    fi
elif [ "$mode" = "restore" ] && [ -f $HOME/printer_data/config/backups/$restore ]; then
    echo "INFO: Restoring $HOME/printer_data/config/backups/$restore ..."
    ...
    backup_files=$(tar -ztvf $HOME/printer_data/config/backups/$restore)
    valid_backup=true
    ...
    if [ -d $HOME/pellcorp-overrides ]; then
        if [ -d $HOME/pellcorp-overrides.old ]; then
            rm -rf $HOME/pellcorp-overrides.old
        fi
        mv $HOME/pellcorp-overrides $HOME/pellcorp-overrides.old
    fi

    if [ -d $HOME/pellcorp-backups ]; then
        if [ -d $HOME/pellcorp-backups.old ]; then
            rm -rf $HOME/pellcorp-backups.old
        fi
        mv $HOME/pellcorp-backups $HOME/pellcorp-backups.old
    fi

    echo "Restoring $restore ..."
    tar -zxf $HOME/printer_data/config/backups/$restore -C $HOME/
    sync
